...
public class OwfCalculator extends Fragment {
	...
	private final static int[] OWF_PAYMENT_STANDARD = {277,380,465,572,671,746,834,924,1017,1108,1198,1290};
	private final static int[] INITIAL_ELIGIBILITY_STANDARD = {487,656,825,994,1163,1333,1502,1671,1840,2009,2178,2348};
	...
	private boolean checkInitialEligibility() {
		...
		if (grossIncomeTotal > INITIAL_ELIGIBILITY_STANDARD[mAGSize]) {
			// if the income is too high, show alert dialog with explanation
			AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
			builder.setMessage("Initial Eligibility Test Failed. The total gross income of $" + grossIncomeTotal + " exceeds the initial eligibility standard of $" + INITIAL_ELIGIBILITY_STANDARD[mAGSize] + " by $" + (grossIncomeTotal - INITIAL_ELIGIBILITY_STANDARD[mAGSize]))
	...
	private boolean checkCountableIncome() {
		...
		if (countableIncome >= OWF_PAYMENT_STANDARD[mAGSize]) {
			...
			builder.setMessage("Countable Income Test Failed. The total countable income of $" + countableIncome + " exceeds the OWF payment standard of $" +
							   OWF_PAYMENT_STANDARD[mAGSize] + " by $" + (countableIncome - OWF_PAYMENT_STANDARD[mAGSize]))
				.setPositiveButton("OK", null)
				.setTitle("Ineligible");
			// Create the AlertDialog object and return it
			builder.create().show();
	...
	private void displayResults() {
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
		builder.setMessage("Eligible for OWF in the amount of $" + (OWF_PAYMENT_STANDARD[mAGSize] - countableIncome) + " per month")
			.setPositiveButton("OK", null)
			.setTitle("Eligible");
		// Create the AlertDialog object and return it
		builder.create().show();
